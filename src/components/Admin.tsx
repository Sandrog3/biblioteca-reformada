...
import { collection, doc, setDoc, getDoc, onSnapshot, addDoc, deleteDoc, updateDoc, serverTimestamp, query, orderBy, where, getDocs } from 'firebase/firestore';
...
export default function Admin() {
  ...
  const isAdminUser = user && (
    user.email?.toLowerCase() === "sandrog3@gmail.com" || 
    role === 'admin'
  );
  ...
  const handleUserSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUserId) {
        ...
        // Check if user already exists to perform an "Upsert" (Update or Insert)
        ...
        if (!snap.empty) {
          ...
          await updateDoc(doc(db, 'users', existingDoc.id), {
            displayName: newUser.displayName || existingDoc.data().displayName,
            role: newUser.role,
            updatedAt: serverTimestamp()
          });
          ...
          showNotification("Usuário autorizado com sucesso!");
        }
      }
      ...
      handleFirestoreError(error, editingUserId ? OperationType.UPDATE : OperationType.CREATE, `users/${editingUserId || 'new'}`);
    } finally {
  ...
  const handleFileUpload = async (
    ...
    type: 'book' | 'product' | 'post' | 'hero' | 'logo', 
    ...
    try {
      ...
      if (fileType === 'image') {
        let aspectRatio = 1;
        if (type === 'book') aspectRatio = 3/4;
        if (type === 'post') aspectRatio = 16/9;
        if (type === 'hero') aspectRatio = 16/9;
        
        try {
          fileToUpload = await processImage(file, { aspectRatio });
        } catch (err) {
          console.error("Erro no processamento, usando original:", err);
          fileToUpload = file;
        }
      }
      ...
      try {
        ...
        showNotification(`${fileType === 'image' ? 'Imagem' : 'Vídeo'} enviado com sucesso!`);
      } catch (error: any) {
        console.error(`Erro no upload do ${fileType}:`, error);
        
        // Fallback for images if Storage fails
        if (fileType === 'image') {
  ...
  useEffect(() => {
          ...
          if (userDoc.exists()) {
            ...
            if (!inviteSnap.empty) {
              ...
              await setDoc(userRef, {
                ...inviteData,
                uid: u.uid,
                displayName: u.displayName || inviteData.displayName || '',
                photoURL: u.photoURL || '',
                lastLogin: serverTimestamp()
              });
              if (inviteDoc.id !== u.uid) {
    ...
    const unsubSettings = onSnapshot(doc(db, 'settings', 'global'), (doc) => {
      if (doc.exists()) {
        ...
        setSiteSettings({
          ...data,
          marqueeItems: Array.isArray(data.marqueeItems) ? data.marqueeItems.join(', ') : (data.marqueeItems || "")
        } as any);
      }
    ...
    if (isAdminUser) {
      unsubSubs = onSnapshot(query(collection(db, 'subscriptions'), orderBy('subscribedAt', 'desc')), (snapshot) => {
  ...
  const login = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/unauthorized-domain') {
        showNotification("Erro: Domínio não autorizado. Adicione 'bibliotecareformada.com' nos domínios autorizados do Firebase Console.", 'error');
      } else {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.code === 'auth/invalid-credential' || error.code === 'auth/user-not-found' || error.code === 'auth/wrong-password') {
        ...
        msg = "O login por e-mail está desativado. Ative 'Email/Password' no Firebase Console > Authentication > Sign-in method.";
  ...
  const saveSettings = async () => {
    setIsSaving(true);
    try {
      const marqueeArray = typeof siteSettings.marqueeItems === 'string' 
        ? siteSettings.marqueeItems.split(',').map(i => i.trim())
        : siteSettings.marqueeItems;
  ...
  const addPost = async () => {
    ...
    try {
      if (editingPostId) {
        ...
        await addDoc(collection(db, 'posts'), { ...newPost, createdAt: serverTimestamp() });
      ...
      handleFirestoreError(e, editingPostId ? OperationType.UPDATE : OperationType.CREATE, 'posts');
    }
  };
  ...
  const addCategory = async () => {
    ...
    try {
      ...
      const categoryData = { ...newCategory, slug };
      ...
      handleFirestoreError(e, editingCategoryId ? OperationType.UPDATE : OperationType.CREATE, 'categories');
    }
  };
  ...
  const addBook = async () => {
    ...
    try {
      ...
      const bookData = { ...newBook, slug };

      if (editingBookId) {
        await updateDoc(doc(db, 'books', editingBookId), bookData);
        setEditingBookId(null);
        showNotification("Livro atualizado!");
      } else {
      ...
      handleFirestoreError(e, editingBookId ? OperationType.UPDATE : OperationType.CREATE, 'books');
    }
  };
  ...
  const deleteBook = async (id: string) => {
    askConfirm("Excluir livro?", async () => {
      try {
        await deleteDoc(doc(db, 'books', id));
        showNotification("Livro excluído!");
      } catch (e) {
  ...
  const addProduct = async () => {
    ...
    try {
      ...
      handleFirestoreError(e, editingProductId ? OperationType.UPDATE : OperationType.CREATE, 'products');
    }
  };
        ...
        <div className="glass p-6 sm:p-12 rounded-[2rem] sm:rounded-[2.5rem] text-center max-w-md w-full border-white/5">
          ...
          {window.location.hostname !== 'localhost' && !window.location.hostname.includes('run.app') && (
            <p className="text-[10px] text-white/20 mt-10 max-w-xs mx-auto leading-relaxed">
              Nota: Se o login do Google não abrir, certifique-se de adicionar o domínio <span className="text-[#D4C3A3]">{window.location.hostname}</span> na lista de "Domínios Autorizados" no Console do Firebase.
            </p>
          )}
          {user && !isAdminUser && (
  ...
  return (
    <div className="min-h-screen bg-[#050505] flex flex-col lg:flex-row text-white">
      ...
      <main className="flex-1 p-4 pt-8 lg:p-12 overflow-y-auto">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 lg:mb-12">
          ...
          <div className="flex items-center gap-4">
            ...
            <img src={user.photoURL || undefined} className="w-10 h-10 rounded-full border border-white/10" alt="Avatar" referrerPolicy="no-referrer" />
          </div>
        </header>
              ...
              <div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                  ...
                  <label className="cursor-pointer px-6 py-3 glass rounded-xl hover:bg-white/10 transition-colors flex items-center gap-2 text-xs font-bold uppercase tracking-widest relative overflow-hidden">
                    ...
                    {isUploading ? "..." : "Upload"}
                    <input type="file" accept="image/*" onChange={(e) => handleFileUpload(e, 'logo', 'image')} className="hidden" disabled={isUploading} />
                  </label>
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                  ...
                  <label className="w-full sm:w-auto cursor-pointer px-6 py-3 glass rounded-xl hover:bg-white/10 transition-colors flex items-center justify-center gap-2 text-xs font-bold uppercase tracking-widest relative overflow-hidden">
                    ...
                    {isUploading ? "..." : "Upload"}
                    <input type="file" accept="image/*,.webp" onChange={(e) => handleFileUpload(e, 'hero', 'image')} className="hidden" disabled={isUploading} />
                  </label>
                </div>
              </div>
              <div>
          ...
          <div className="space-y-12">
            <div className={`glass p-8 rounded-3xl transition-all ${editingCategoryId ? 'border-[#8B5E3C] ring-1 ring-[#8B5E3C]/50' : ''}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div className="col-span-1 md:col-span-2">
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-white/30 mb-1 ml-1">URL da Imagem de Capa (Opcional)</label>
                  <input 
                    placeholder="URL da imagem para a vitrine" 
      ...
      {/* Notifications */}
      {notification && (
        <div className={`fixed bottom-8 right-8 px-6 py-4 rounded-2xl shadow-2xl z-[100] flex items-center gap-3 animate-in fade-in slide-in-from-bottom-4 duration-300 ${
